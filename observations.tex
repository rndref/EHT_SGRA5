...
For each model we generate a distribution of $\mi{3}$.  We then use a two-sample Kolmogorov-Smirnov (KS) test to estimate the probability $p$ that the model distribution and observed distribution are drawn from the same underlying distribution.  We reject the model if $p < 0.01$.

The strength of the $\mi{3}$ constraint depends on the number of samples in both the data and the model because we use a two sample KS test to assign a $p$ value.  The fiducial models have duration $10^4$ or $1.5\times 10^4 \tg$, whereas most exploratory models have duration $5 \times 10^3 \tg$ and thus fewer $\mi{3}$ samples. The $\mi{3}$ constraint is therefore weaker for the exploratory models; some pass the constraint but that does not mean they are less variable.
