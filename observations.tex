...
\cfg{heavily edited to make consistent with length and tone of other constraints.}

Our first constraint checks the morphology of the visibility amplitudes.  We impose two requirements on the model: is the first minimum in the visibilities in about the right place?  and are the long-baseline visibility amplitudes less than a factor of a few larger than the data?  The location and depth of the minima in the visibility amplitudes, for example, depends on the image size and  also the ring thickness (if there is a ring).

We use data from \aprilvii, which has the best u-v coverage near minima in the visibility amplitudes.  The first visibility minima in both the N-S and E-W directions occur between $2.5-3.5$\;G$\lambda$.  We require that the model have $|$
