...
We test the models using interferometric data in three ways.  First, we compare the location of the first null and the visibility amplitude of long baselines to the model (``null constraint'').  Second, we compare an estimate of the source size (``second moment constraint'') from short baselines to the model.  Finally, we follow a variant of the procedure used in \citetalias{PaperIV} and compare fits for the diameter, width, and asymmetry of an m-ring to the model.  
...
near the minima.\monika{I think the first sentence should be mentioned in the first paragraph in section 2.1}
...
Next, we read in a series of model images, generated synthetic data from the models, and fit m-rings to the synthetic data.  This produced a distribution of m-ring parameters for each model.   
...
We compute 86GHz images for all models, and from that the 86GHz flux density $F_{86}$.  We implicitly assume that the 86GHz variability over spans of order $3$days is sampled during the model integration.  

We reject models if $< 1\%$ of model snapshots have $1.8 < F_{86} < 2.2$Jy.
\cfg{We are likely to change this to read: we convolve the model $F_{86}$ distribution with a distribution corresponding to $0.1$Jy errors, and reject the model if the resulting CDF $< 1\%$ or $> 99\%$ at $2.0$Jy.}
...
We compute the major axis FWHM from the model images.  Again assuming that the models are run sufficiently long to sample the full range of 86 GHz major axis sizes, we reject models if $< 1\%$ of model snapshots lie within the 95\% limits.   
...
We compute $2.2\mu$m images for all models.  We reject the model if the median flux exceeds the observed median flux.  

We do not require that the model produce NIR flares, for which there is as yet no generally accepted model.  Our working hypothesis is that models that underproduce NIR and which do not flare can be saved by accelerating a small fraction of the electron population into a nonthermal tail.  

%------------------------------------------------------------------------------
\subsubsection{X-ray (Non-Overproduction) Constraints}

\sgra flares in the X-ray less frequently than in the NIR.  Chandra observations during the 2017 campaign 
...
Compared to other astrophysical systems, optically thin black hole accretion systems such as \sgra are expected to show variabilities in an extremely wide range of time scale, from the "environment" determined boundary conditions at the Bondi radius, all the way to the turbulent fluctuations near the horizon scale, or even plasma instability driven phenomena.  Even excluding flares and detailed plasma physics, observations have shown intrinsic variability in \sgra~\citep{sgra lightcurve papers}.  The standard theoretical interpretation is that the turbulence in the accretion flow drives the fluctuations in, e.g., temperature and magnetic fields, which then drive the fluctuations in the electromagnetic signals.  Assuming that the magnetorotational instability is the driving mechanism, the turbulence integrated time scale is simply the orbital time scale.  Given that synchrotron radiation mainly comes from the inner accretion disk, this time scale is of order of 10 minutes.  However, the chaotic nature of turbulence makes these short time scale variability extremely difficult to predict.  On the other hand, given the slowly varying boundary conditions, numerical simulations are very successful in predicting slowly varying the mean images, SEDs, etc.  Because of this, the constraints described above use only the mean/median quantities.  And we will only discuss the variability, but not use them to rule out models.
