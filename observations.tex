...
baselines.  This is done without any assumption about the structure of the source;
...
$135^\degree$ (because of Hermitian symmetry we need only cover $0-180\deg$).
...
Here we must make a choice about which lightcurve data to use.  One possibility is to use the $\mi{3}$ measured from the E

For each lightcurve longer than 3 hours in the ALMA data and in earlier SMA and CARMA data (the ``historical dataset''; see \citealt{Wielgus2022}), we take 3 hour segments equally spaced away from the endpoints and each other and calculate  $\mi{3}$ on each segment. This procedure gives us a set of 42 samples of the $\mi{3}$ for the historical dataset.  Other choices for the comparison dataset are possible and the choice is subjective.  We note that the $\mi{3}$ measured from the EHT 2017 observations on April $5-11$ are consistent with being drawn from the historical distribution, although April 7 has one of the quietest segments on record, and April 11 has one of the most variable.  

For each model we make a synthetic lightcurve and from that generate a distribution of $\mi{3}$.  We then use a Kolmogorov-Smirnov (KS) test to ask whether that distribution is consistent with the historical distribution, and reject the model if $p < 0.01$.
