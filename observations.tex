...
The minima locations and long-baseline amplitudes are sensitive to the
source structure.
...
the ring diameter, while the VAs on long-baselines depends mainly on
ring width.
GRMHD models are more complicated, with significant structural
fluctuations.
The minima locations and depths, and long-baseline amplitudes, are
...
the VAs has $\lesssim 4\%$ of the zero-baseline flux.
...
We find the first minima numerically and compute the median VAs
...
\emph{ii})~at no position angle does the median VAs exceed $4\% /
...
\sgra flares in the x-ray less than about once per day \citep{...}.  Chandra observations during the 2017 campaign suggest an upper limit on the median (quiescent) $\nu L_\nu$ at $6\,\mathrm{keV}$ of $10^{33}\ergsps$ \citep{PaperIII}.
...
\begin{figure*}
  ...
  \caption{%
    Visual representation on how the second moment constraint work.
