...
\sgra~\citep[e.g.,][]{2006MNRAS.370..219M, 2007MNRAS.379.1519M,
  ...
  2020arXiv200514251B, 2020MNRAS.tmp.2416D, 2020arXiv200603658P}.
...
%==============================================================================
\section{Fiducial Models of \sgra}
\label{sec:fiducial}
...
The large number of observation and theoretical studies on \sgra have
already lead to many robust constraints.

In this section, we summarize various results on \sgra using
pre-EHT2017 data.

We define a minimal set of fiducial models that can be used to coverge
a wide range of phenomena.

We start with an one-zone model in \S\ref{sec:onezone}, which will
give us order of magnitude estimate of the physical properties of
\sgra.
We will ...
For the innermost accretion disks, GRMHD and GRRT will be used to
capture the dynamics, thermodyanmics, and ...

We summarize some phenomenological models in \S\ref{sec:phenomodels}.
...
...
In this paper, we adopted a weighted mean of
\citet{2019Sci...365..664D} and \citet{2019A&A...625L..10G} and fix
the mass of \sgra to
\begin{align}
...
\ckc{Is this the right place to put disk vs. jet discussion?
  ...
  Recent paper: \citep{2020arXiv200804317Y}.}
...
While GRMHD simulations provide numerical models of the plasma around
black holes, there are many more parameters needed to model the black
hole images and spectral energy distribution (SED).
This is because, obviously, the GRMHD approximaion is scale free in
both the black hole mass $\mbh$ and plasma density $\rho_0$.
The former allows us to rescale length and time, while the later
allows us to rescale magnetic field strength and energy density.
...
Therefore, it is expected that \emph{i}) electrons are cooling than
the ions, and \emph{ii}) the electron distribution function (eDF) is
non-thermal.
The modeling details can signficiant change the predcited properties
of \sgra~\citep{2000ApJ...541..234O}.

%------------------------------------------------------------------------------
\subsection{Plasma Models}
\label{sec:eDF}

Many different model were used to approximate the eDF.
...
\ckc{Should we directly compare reconstructed images and simulated
  images by, e.g., \texttt{nxcinf}.}
...
%------------------------------------------------------------------------------
\subsection{Image Size (Pre-Imaging) Constraints}
\label{sec:sizeconst}
...
%------------------------------------------------------------------------------
\subsection{Light Curve Covariance}
\label{sec:lcconst}
