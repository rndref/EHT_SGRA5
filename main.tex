...
\begin{abstract}
  ...
  In this paper, we provide a first physical interpretation for the 2017 EHT observations of \sgra.  Our main approach is to compare resolved EHT data and unresolved non-EHT data to a library of time-dependent general relativistic magnetohydrodynamics (GRMHD) models, including aligned, tilted, and stellar wind-fed models; the models are ray-traced assuming both thermal and nonthermal electron distribution functions.  We test the models against 11 constraints drawn from EHT 230\GHz data and observations at 86\GHz, 2.2\um, and in the X-ray.  None of the  models explain all the data.  We identify two locations in parameter space, however, where the models explain nearly all the data, and identify possible reasons for the failure.  In particular we identify a promising cluster of three prograde, strongly magnetized (MAD) models at $i \le 30$ that survive nearly all constraints.  These models have accretion rate $5.2$--$9.5\times10^{-9}\msun\yr^{-1}$, bolometric luminosity $6.8$--$9.2\times10^{35}\ergps$, and outflow power $1.3$--$4.8\times10^{38}\ergps$.  A surprising 
  
  
  We also find that most of our nonthermal model sets produce 230\GHz images that are observationally indistinguishable from thermal models but have uniformly higher 2.2\um flux density; that all models with equal ion and electron temperatures are rejected; and that the population of cold electrons is limited by X-ray constraints due to the risk of bremsstrahlung overproduction.  A few models satisfy nearly all the constraints, but this work signals the arrival of a new era of precision black hole astrophysics in which theoretical models are strongly challenged by the data.
