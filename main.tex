...
\begin{abstract}
  ...
  The scinetific legacy of this paper may include:
  %
  \emph{i}) Describe a standard numerical methods that are used in
  EHT's \sgra papers.
  E.g., the ``grmhd'' test set for imaging, calibration sets for MCFE
  and gravity.
  %
  \emph{ii}) Provide the first estimate on theoretical uncertainties.
  I.e., the ``systematic error'' in theoretical modelings.
  %
  \emph{iii}) Provide the first theoretical/numerical interpretation
  of the EHT \sgra VLBI observation, with helps from historical data.
  %
  \emph{iv}) Gather a legacy observation data set that \emph{all
  future} theoretical studies should include.
  %
  \emph{v}) Outlook into future theoretical research directions, i.e.,
