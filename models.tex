...
\begin{figure*}
  ...
  \caption{Sample snapshot 230\GHz images from the simulation library:
    (\emph{left})~a thermal SANE example,
    (\emph{middle})~a thermal MAD example, and
    (\emph{right})~a non-thermal variable $\kappa$ MAD.}
  \label{fig:sample_230GHz}
\end{figure*}
...
The plasma mass scale parameter $\Munit$ controls the optical depth and thus the source brightness.  We adjust $\Munit$ iteratively until the time-averaged 230\GHz flux densities of the models are within a few percent of the $2.4\,\mathrm{Jy}$ mean observed during the 2017 campaign (see next section).  The reader should keep in mind that the model trends discussed in Section~\ref{sec:trends} assume constant time-averaged millimeter flux density.
...
\begin{figure*}
  ...
  \caption{Sample snapshot 86\GHz images from the simulation library:
    (\emph{left})~a thermal SANE example,
    (\emph{middle})~a thermal MAD example, and
    (\emph{right})~a non-thermal variable $\kappa$ MAD.}
  \label{fig:sample_86GHz}
\end{figure*}
...
In Figures~\ref{fig:sample_230GHz}, \ref{fig:sample_86GHz}, and \ref{fig:sample_SEDs}, we present a representative set of model images and multiwavelength SEDs from our library.
