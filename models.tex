...
\begin{figure*}
  ...
  \caption{3-D overview of selected GRMHD simulations of \sgra in our library.
    ...
    In case of spinning black holes, the spin is aligned with z-axis.\monika{colors will be changed in a jiffy, im on it}
  }
  \label{fig:GRMHD}
\end{figure*}
...
\begin{figure*}
  ...
  \caption{Sample 86\GHz (top row) and 230\GHz (bottom row) images
    from the simulation library: the left column shows a thermal SANE,
    middle column shows a thermal MAD, and the right column shows a
    nonthermal variable $\kappa$ MAD.
    Note that these models are simply chosen to demonstrate the
    variety of possible images.
    They do not necessary pass all the constraints.
    The image library is constructed by normalizing the 230\GHz flux
    at $\simeq 2.4\,\mathrm{Jy}$.
    On one hand, most of the 230\GHz images are in an optically thin
    regime, making the sharp photon ring visibile (bottom left and
    middle panels).
    On the other hand, most of the 86\GHz images are in an optically
    thick regime, and the accretion disk would block the sharp photon
    ring (top middle and right panels).
    In general, face-on images like the ones in the left column tend
    to be too big in size, and the edge-on images in the right column
    tend to be too asymmetric.}
...
\begin{figure*}
  ...
  \caption{SEDs from the same sample models shown in
    ...
    The black dots and arrows in the panels indicate the required
    fluxes at 86\GHz and 230\GHz, and the upper limits at NIR and
    x-ray, respectively.
    ...
    bremsstrahlung, see the legend).
    The total emission is marked as black spectrum.
    The left panel shows a interesting special case in our simulation
    library, where the spectral index at 230\GHz is negative.
    This indicates the 230\GHz image is completely optically thin and
    the 86\GHz would over produce the flux.
    The NIR has a flat spectral index and is dominated by inverse
    Compton, instead of direct synchrotron radiation.
    These properties occur only for $\Rh = 1$ SANE models.
    For $\Rh > 1$, the direct synchrotron peak shifts to the right,
    broadens its width, and dominates the NIR.
    For SANE, the spetral index for x-ray is almost always positive
    and dominated by thermal bremsstrahlung by (relatively) cool
    electrons.
    The middle and right panels show the more typical MAD SEDs, where
    86\GHz, 230\GHz, and NIR all come from direct synchrotron, and
    x-ray is dominated by inverse Compton.}
...
All radiative transfer models used here set the emissivity to $0$ at $\sigma \equiv B^2/(8\pi\rho c^2) > \sigma_\mathrm{cut} = 1$.  This is motivated by the notion that the numerical solution is unreliable at $\sigma > \sigma_\mathrm{cut}$.  In particular numerical diffusion into the low density region (at the ``funnel wall'') makes the density too large, and truncation error in integration of the total energy equation produces large fractional errors in temperature.
