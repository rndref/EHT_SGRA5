...
The plasma mass scale $\Munit$ is a free parameter and control the opacity depth and hence the images and SEDs.
In order to reduce the parameter space, we employ an iterative process to solve for $\Munit$ so that the mean 230\GHz flux densities of the models match roughly the $2.4\,\mathrm{Jy}$ observed mean from the 2017 campaign (see next section).
Note that the flux is a nonlinear function of $\Munit$ because the accretion flow transits between the optical thick and thin regimes around 230\GHz.
...
The models are imaged at 86\GHz, 230\GHz\footnote{The mean frequency for  EHT observations is more close to 228\GHz.} and 2.2\um (near infrared, NIR hereafter).
