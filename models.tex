...
\end{equation}
The expected diameter of the black hole shadow is $2\sqrt{27} G\mbh/(c^2 D)$ for $\abh = 0$.  For $|\abh| > 0$ the shadow is noncircular and its size and shape depend on $\abh$ and inclination $i$; its width can be as small as $9 G\mbh/(c^2 D)$ for $\abh = 1$ and $i = 90\deg$ \citep{1973blho.conf..215B}.
...
In what follows we assume that radiative cooling of plasma around the black hole can be neglected (which is partially justified later) and that the emergent radiation can be calculated in a post-processing.

%==============================================================================
\subsection{One-Zone Model Estimates}

Here we use a one-zone model to motivate the more complicated models that follow, as in \citetalias{M87PaperV}.  These results follow one-zone models developed in the literature over many decades \citep[e.g.][]{1996IAUS..169..169F}.
...
The magnetic field is set so that the gas pressure and magnetic pressure are proportional: $n_i k T_i + n_e k T_e = \beta B^2/(8\pi)$, where $T_i \equiv$ ion temperature, $T_e \equiv$ electron temperature, $B \equiv$ magnetic field strength in Gauss.  We assume $\beta=1$, $T_i = 3 T_e$ (a two-temperature plasma), and $\Theta_e \equiv  k T_e / m_e c^2 = 10$ (relativistic electrons and nonrelativistic ions).
...
\begin{deluxetable*}{cccccccc}\label{tab:GRMHDmodels}
  ...
  \startdata
  torus & {\kharma}$^a$ &
   0,$\pm1/2$,$\pm15/16$  & MAD/SANE     & $\frac{4}{3}/\frac{4}{3}$      & 30,000  & 1000     & 288x128x128
  \\
   torus & \bhac$^b$   & 0,$\pm1/2$,$\pm15/16$                 & MAD/SANE     & $\frac{4}{3}/\frac{4}{3}$      & 30,000  & 3333     & 512x192x192 \\
  torus & \hamr$^c$ & 0,$\pm1/2$,$\pm15/16$                 & MAD/SANE     & $\frac{13}{9}/\frac{5}{3}$ & 35,000  & 1000/200 & 348/240×192×192 \\
  torus & \koral$^d$  &  0,$\pm0.3$,$\pm0.5$,$\pm0.7$,$\pm0.9$ 
  & MAD          & $\frac{13}{9}$     & 101,000 & 100,000  & 288x192x144 \\
   tilted  & \hamr$^e$   & $15/16$     & IN-SANE      & $\frac{5}{3}$        & 105,000 & 100,000  & 448x144x240  \\
   wind-fed  & \athenapp$^f$ & 0           & MAD & $\frac{5}{3}$        & 20,000  & 2,400    & 356x128x128  
  \enddata
  \tablecomments{Summary of GRMHD simulations in the EHT \sgra GRMHD simulation library.
    The first four entries use aligned torus initial conditions.
    The last two entries are tilted accretion models and two realizations of the Wind-fed accretion model which differ in stellar wind magnetization.  Times are given in units of $G M/c^3 = 20.4$sec and radii in units of $G M/c^2$.}
\tablerefs{
$^a$ see \cite{2021JOSS....6.3336P}; \kharma is a GPU-enabled version of the {\tt iharm3d} code. 
$^b$ \cite{2017ComAC...4....1P, 2019A&A...629A..61OF}. 
$^c$ \cite{2021arXiv210812380N}. 
$^e$ \cite{arXiv:1912.10192, Chatterjee2020}. 
$^f$ \cite{2016ApJS..225...22W, 2020ApJ...896L...6R}.
}
\end{deluxetable*}
...
The initial condition for most of our GRMHD simulations is a hydrodynamic, constant-angular-momentum equilibrium, the Fishbone-Moncrief torus \citep{1976ApJ...207..962F}. Although there are variations in initial conditions between simulations, in all except the \hamr-Tilted models, the torus orbital angular momentum is either parallel or antiparallel to the black hole spin.
...
It is well established (see, e.g, \citealt{M87PaperV} and \citealt{M87PaperVIII}, hereafter \citepalias{M87PaperVIII}, and references therein) that the magnetic flux through the event horizon divides the outcomes into two categories: the magnetically arrested disk (MAD) state \citep[e.g.,][]{1974Ap&SS..28...45B, 2003ApJ...592.1042I, 2003PASJ...55L..69N} in which the magnetic flux on the horizon saturates and substantially affects the dynamics of the flow, and the Standard and Normal Evolution (SANE) state \citep[e.g.,][]{2003ApJ...589..444G, 2003ApJ...599.1238D, 2012MNRAS.426.3241N}.
...
We also consider two GRMHD simulations with different initial conditions: strongly magnetized non-MAD tilted torus simulations \citep{Liska2018, Chatterjee2020} and a model in which \sgra is fed directly by winds from stars in its vicinity \citep{2020ApJ...896L...6R}.
...
This model has three free parameters: $\beta_\mathrm{crit}$, $\Rl$, $\Rh$.  We fix $\Rl = 1$ and $\beta_\mathrm{crit} = 1$, but allow $\Rh$ to vary from 1 to 160. %\cmf{This parameterised electron temperature prescription is well matched to electron temperature distribution obtained by two-temperature GRMHD simulations including electron heating prescription \citep{Mizuno2021}.}\monika{has that been checked for Sgr A* conditions in Mizuno paper?}{\color{red}[AC:] add reference to Paper 8, where we compare Rhigh models directly to two-temperature simulations (of M87). There we do \emph{not} find that Rhigh/low is always a good description, but see more complicated structure in the temperature ratio from two-temperature.}\gw{I'll note that I did a comparison to various simulations (ebhlight and iharm3d) with Howes and Kawazura electrons and also did not find good agreement.}
...
%%   \caption{Example SEDs from the models shown in
%%     Figure~\ref{fig:example_imgs}:
...
%%     \ckc{TODO: fix brems (once) SED in the middle panel; udpate Frankfurt SED (right).}
%%     }
...
Given an eDF, density scale $\Munit$, and radiative transfer coefficients based on local properties of the plasma, the emergent radiation is obtained by integrating the radiative transfer equation.
...
%\begin{rotatetable}
\begin{deluxetable*}{cccccccc}\label{tab:radiativemodels}
%\tabletypesize{\footnotesize}
\tablecaption{EHT Synthetic Image Library}
%\renewcommand{\arraystretch}{1.1}
\tablehead{
  \colhead{Simulation}              &%
  \colhead{$\Rh$}              &%
...
\startdata
\multicolumn{8}{c}{\bf Fiducial models}\\
\hline
\kharma &  1, 10, 40, 160 &  - & - & 10, 30, ..., 170 & 86, 230 & Yes & 15-30 \\
\bhac  &  1, 10, 40, 160 &  - & - & 10, 30, ..., 90  & 86, 230 & Yes & 20-30 \\
\hamr  &  1, 40, 160     &  - & - & 10, 50, 90       & 86, 230 & Yes & 30-35 \\
\koral &  20             &  - & - & 10, 30, ..., 170 & 86, 230 & No  & 5-100 \\
\hline
\multicolumn{8}{c}{\bf Exploratory models}\\
\hline
\multicolumn{4}{l}{\it Thermal $\Rh$ models} & & & & \\
\hamr Tilted &  1, 40, 160  & - & - & 10, 50, 90     & 86, 230 & Yes & 100-103 \\
Wind Accretion & 13, 28  &  - & - & N/A     & 86, 230 & No  & 10        \\
\hline
\multicolumn{4}{l}{\it Thermal critical $\beta$ model} & & & & \\
\kharma & 1, 40, 160 &  4 & - & 10, 50, 90 & 86, 230 & No & 30-35  \\
\hline
\multicolumn{4}{l}{\it Thermal + power-law models} & & & & \\
\hamr &  1, 40, 160 &  4 & - & 10, 50, 90 & 86, 230 & No & 30-35  \\
\hline
\multicolumn{4}{l}{\it Thermal + $\kappa$ models} & & & & \\
\bhac & 1, 10, 40, 160 &  - & 5  & 10, 30, ..., 90 & 86, 230 & No  & 25-30  \\
\bhac & 1, 10, 40, 160 &  - & 3.5 ($\epsilon_0=0.05$) & 10, 30, ..., 90 & 86, 230 & No  & 25-30 \\
\bhac & 1, 10, 40, 160 &  - & 3.5 ($\epsilon_0=0.10$)  & 10, 30, ..., 90 & 86, 230 & No  & 25-30  \\
\bhac & 1, 10, 40, 160 &  - & 3.5 ($\epsilon_0=0.20$) & 10, 30, ..., 90 & 86, 230 & No  & 25-30  \\
\bhac & 1, 10, 40, 80, 160 &  - & variable $\kappa=\kappa(\beta, \sigma)$ & 10, 30, ..., 90 & 86, 230 & No  & 25-30  \\
\hamr & 1, 10, 40, 160 &  - & variable $\kappa=\kappa(\beta, \sigma)$ & 10, 30, ..., 90 & 86, 230 & Yes & 30-35  \\
\enddata
\tablenotetext{
Summary of emission simulations in the EHT \sgra model library. For the Wind Accretion model the viewing angle is set by the boundary conditions used in the model and $\Rh$ is set so the model matches the observed 230\GHz flux.  The two reported values correspond to two models with different stellar wind magnetizations.
}
