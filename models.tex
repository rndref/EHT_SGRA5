...
The synchrotron cooling timescale is $t_\mathrm{cool} \equiv u/\Lambda$ where $u_e = 3 n_e k T_e$ is the electron internal energy and $\Lambda \simeq 5.4 B^2 e^4 n_e \Theta_e^2 /(c^3 m_e^2)$ is the synchrotron cooling rate from thermal population of electrons (for details see Appendix~A in \citealt{2011ApJ...735....9M}; finite optical depth would reduce $\Lambda$).
...
As demonstrated in \citet{2000ApJ...541..234O}, \citet{2009ApJ...701..521C}, and \citet{2014A&A...570A...7M} \citep[see also more recent work by][]{2018A&A...612A..34D, 2021A&A...650A.163C, 2021arXiv211102518F, 2021NatAs.tmp..218C, 2021arXiv211203933E}, both effects may change the predicted properties of \sgra.
...
\begin{deluxetable*}{cccccccc}
  ...
  \startdata
  \kharma & 0,$\pm1/2$,$\pm15/16$                 & MAD/SANE     & $4/3$      & 30,000  & 1000     & [288x128x128]     & \!\!\!\!\!\!\!\!\!This work, \citet{Wong_2022, Dhruv_2022} \\
  ...
  \koral  & \!\!\!\!\!\!\!\!\!0,$\pm0.3$,$\pm0.5$,$\pm0.7$,$\pm0.9$\!\!\!\!\!\!\!\!\! & MAD          & $13/9$     & 101,000 & 100,000  & [288x192x144]     & \citet{2021arXiv210812380N} \\
  ...
  \caption{Summary of GRMHD simulations in the EHT \sgra GRMHD model library.
    ...
    There are also tilted accretion models and two realizations of the Wind Accretion models which differ in stellar wind magnetization.}
  \label{tab:GRMHDmodels}
\end{deluxetable*}

The one-zone model is too simple for comparison with the rich set of observational constraints available for \sgra.
Analytic spherical accretion models \citep[e.g.][]{2019ApJ...885L..33N, 2021arXiv211102178B} incorporate black hole gravity, and
analytic disk-like (RIAF) accretion models in the Kerr metric \citep[e.g.][]{2009ApJ...697...45B, 2009ApJ...706..960H,2018ApJ...863..148P}
can in principle be used for extensive plasma parameter surveys (see Section~\ref{sec:future}).
However, they do not self-consistently capture important physics such as turbulent transport.
Therefore, we simulate the plasma flow with an ideal GRMHD\footnote{Note that ideal GRMHD models do not capture collisionless aspects of the accretion flow and jets; we discuss the model limitations in Section~\ref{sec:limits}.} model, assign an electron distribution function, and solve the radiative transfer equation along geodesics to produce synthetic observations.
...
\aco{We should specify the torus size? or should we refer to the reader to SANE and MAD code comparison.
  We have different initial data between codes, not only in the size but also in adiabatic index in the equation of state.}
...
It is well established \citep[see, e.g.,][and references therein]{M87PaperV, M87PaperVIII} that the magnetic flux through the event horizon divides the outcomes into two categories: the Magnetically Arrested Disk (MAD) state \citep[e.g.,][]{1974Ap&SS..28...45B, Igumenschchev:2003, 2003PASJ...55L..69N} in which the magnetic flux near the horizon saturates and significantly affects the dynamics of the flow, and the Standard and Normal Evolution (SANE) state \citep[e.g.,][]{2003ApJ...589..444G, 2003ApJ...599.1238D, 2012MNRAS.426.3241N}.
...
The self-consistent wind feeding simulations result in a mode of accretion that is somewhat similar to MAD but typically has lower mean angular momentum and is less well organized.
...
Motivated by plasma heating models that energy dissipation to ions and electrons depends on the local magnetic properties \citep[e.g.,][]{2010MNRAS.409L.104H, Kawazura771},
we use a prescription with the assumption that the temperature ratio is a local function of the plasma $\beta \equiv P_\mathrm{gas}/P_\mathrm{mag}$ \citep{2015ApJ...799....1C}.
Specifically, we use the same model as in \citetalias{M87PaperV} and \citetalias{M87PaperVIII}, where $R$ is a smooth function adopted from \cite{2016A&A...586A..38M}:
\begin{equation}
...
In the standard models we will present in this paper, we fix $\Rl = 1$ and $\beta_\mathrm{crit} = 1$, and only allow $\Rh$ to vary from 1 to 160.

In \emph{non-thermal} models, the eDF has a power-law tail extending to high energy.
We explore two implmentations in this paper:
\emph{i}) with a power-law distribution function
\begin{align}
  \frac{1}{n_e} \frac{d n_e}{d\gamma} &=
  \frac{p-1}{\gamma_{\min}^{1-p} - \gamma_{\max}^{1-p}}
  \gamma^{-p}
  \label{eq:nonthermaleDF}
\end{align}
which has power-law index $p$ and upper and lower limits $\gamma_{\min}$ and $\gamma_{\max}$; and
...
Evidently, any eDF assignment schemes is an approximation in the sense that the eDF will in general depend on both local conditions and particle histories.
The eDF is assumed isotropic, and electron-positron pairs are neglected in this work.
Nevertheless, see Section~\ref{sec:pair} for a discussion of potential impact of pair plasma for \sgra.
...
With the exception of the special stellar wind-fed simulations, the GRMHD models considered in this work are scale-free.
Physical units are assigned during the radiative transfer step.
The black hole mass $\mbh$ fixes the length unit $\rg$ and time unit $\tg$.
Because the GRMHD models are not self-gravitating, one is free to set a density scale, or equivalently the accretion rate $\dot{M}$ or plasma mass unit $\Munit$.
...
\ckc{I found this notation confusion.  For example, $c$ is dimensional, although it's not necessary in cgs.  So I'm not sure what $c_\mathrm{cgs}$ really means here.  Is the dimensionless value of $c$ in cgs?  I'm probably too picky...}

The plamas mass scale $\Munit$ is a free parameter and control the opacity depth and hence the images and SEDs.
In order to reduce the parameter space, we employ an iterative process to solve for $\Munit$ so that the mean 230\GHz flux densities of the models match the $\simeq 2.4\,\mathrm{Jy}$ observed mean from the 2017 campaign (see next section).
Note that the flux is a nonlinear function of $\Munit$ because the accretoin flow transits between the optical thick and thin regimes around 230\GHz.
This makes the interpretations of the model trends less obvious.

%..............................................................................
\subsubsubsection{Radiative Transfer Calculation}

Given an eDF, density scale $\Munit$, and radiative transfer coefficients, the emergent radiation is obtained by solution of the radiative transfer equation.
For this we use two classes of numerical methods: ray tracing to generate synthetic images, and a Monte Carlo scheme to generate spectral energy distributions (SEDs).
Further detail on numerical methods is given Appendix~\ref{app:radtrans}.
Comparisons of numerical methods \citep[][]{Prather_et_al_2021, 2020ApJ...897..148G} shows that differences between numerical schemes do not contribute significantly to the error budget.

The models are imaged at 86\GHz, 230\GHz\footnote{The mean frequency for  EHT observations is more nearly 228\GHz.} and 2.2\um.
Direct imaging includes synchrotron emission and absorption and bremsstrahlung (both ion-electron and electron-electron; see \citet{2020ApJ...898...50Y} for a recent review).
The SED is averaged over a narrow range in inclination and over all position angles.
The SED includes synchrotron, bremsstrahlung, \emph{and} Compton scattering.
NIR emission is usually dominated by synchrotron, but we find that occasionally NIR synchrotron is so weak that Compton scattering dominates.
The X-ray is dominated by bremsstrahlung or Compton scattering.
...
\begin{figure*}
  ...
  \caption{Snapshot 86\GHz and 230\GHz images and corresponding time-averaged SEDs from the simulation library:
    ...
    \ckc{will do.  86GHz currently uses lower resolution.  We can either interpolate, or increase the resolution for these fudicial models.}\monika{CK can you check label in the middle panel? is middle really mad?}\aco{Which  quantity we are plotting and in which range (in the colormap)?}}
