...
The development of complex models is guided by simple estimates. Following \citetalias{M87PaperV} we consider a one-zone model for \sgra. The model consists of a uniform plasma sphere of radius $5 \rg$ with magnetic field oriented at $\pi/3$ to the line-of-sight. The magnetic field in the sphere is in pressure equilibrium with the plasma: $n_i k T_i + n_e k T_e = \beta B^2/(8\pi)$, where we assume $\beta=1$ and $T_i = 3 T_e$.  Assuming the dimensionless electron temperature $\Theta_e = kT_e / m_e c^2 = 10$, using the thermal emissivity of \cite{2011ApJ...737...21L}, assuming optically thin emission, the values of $B$ and $n_e$ required to produce observed flux of 2 Jy (average flux of \sgra measured by ALMA during the 2017 campaign) are : \color{green}\begin{eqnarray}
\label{eq:onezone}
