...
Because the GRMHD models are not self-gravitating, one is free to set a density scale, or equivalently the accretion rate $\dot{M}$ or plasma mass unit $\Munit$ \citep[see, e.g.,][for a full discussion]{Wong_2022}.   

% cfg: resolved this by reference to the PATOKA paper.  We really don't need all this anywhere else in the paper.
...
The plasma mass scale parameter $\Munit$ controls the optical depth and thus the source brightness.  We employ an iterative process to solve for $\Munit$ so that the time-averaged 230\GHz flux densities of the models are within a few percent of the $2.4\,\mathrm{Jy}$ mean observ the 2017 campaign (see next section).
