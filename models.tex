...
The density in cgs units ($\rho_\mathrm{cgs}$) is obtained from the density  in simulation units ($\rho_{sim}$) via $\rho_\mathrm{cgs} = \rho_{sim} \Munit_\mathrm{cgs} {(\rg)}_\mathrm{cgs}^{-3}$.  Similarly, $u_\mathrm{cgs} = u_{sim} \Munit_\mathrm{cgs} {(\rg)}_\mathrm{cgs}^{-3} c_\mathrm{cgs}^2$, and $B_\mathrm{cgs} = (4\pi)^{1/2} B_{sim} \Munit_\mathrm{cgs}^{1/2} {\rg}_\mathrm{cgs}^{-3/2} c_\mathrm{cgs}$; the factor of $(4\pi)^{1/2}$ comes from converting Heaviside-Lorentz units (used in GRMHD simulations) to cgs.\monika{what is $\rho_{unit}$ in Table 2? do we want to keep this parameter; if yes then what does it mean?}
