...
\begin{figure*}
  ...
  \caption{% Sample 86\GHz (top row) and 230\GHz (bottom row) images
    from the simulation library: (\emph{left})~a thermal SANE,
...
\begin{figure*}
  ...
  \caption{%
    ...
    The left panel shows a interesting case, where the spectral index
    at 230\GHz is negative, indicating the 230\GHz image is completely
    optically thin and the 86\GHz would over produce the flux.
    The NIR has a flat spectral index and is dominated by inverse
    Compton, instead of direct synchrotron radiation.
    Finally, the spetral index for x-ray is positive, indicating the
    x-ray emission is dominated by thermal bremsstrahlung by
    (relatively) cool electrons.
