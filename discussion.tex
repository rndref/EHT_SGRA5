...
\monika{some of the collisionless aspects are addressed in appendix C1 in subsection on selfconsistent electron heating models. check if this is mentioned below}\br{ideal grmhd, not even two-temperature grmhd, cannot capture collisionless aspects, particularly not the ones we are talking about here. we can add a line on two-temperature grmhd, but the main limitation we are talking about is non-thermal distributions, which 2T grmhd also cannot capture. So neither non-thermal edf, nor collisionless aspects like pressure anisotropy, nor resolved (collisionless-alike) heating mechanisms like turbulence or reconnection can be captured with 2T grmhd. I think the main point of this section is to clarify why grmhd is not the set of equations that we actually need, and why, but that it is hard to do grpic or non-ideal grmhd (i.e. with viscosity, resistivity, heat conduction) or full two-fluid MHD (i.e. evolving both fluids, very different from two-temperature grmhd). And two-temperature GRMHD is not self-consistent, it takes heating from the grid that goes into the electron temperature (which cannot go non-thermal in this way anyway), so it is in no way ``collisionless''}
