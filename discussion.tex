...
Evidently adding a population of nonthermal electrons does not provide a consistent way of transforming failing fiducial models to passing models.  
...
Diffuse emission on scales larger than the VLBI images ($\sim 100\uas$) and smaller than connected element interferometer measurements ($\sim 100\uas$) is difficult to constrain.
...
% cfg 2/5: since we're not providing the terminals anymore
...
%This can be understood as a consequence of three facts:
%\emph{i})~the thermal synchrotron emissivity is an increasing function of temperature, so at fixed flux density higher electron temperature models have lower $\dot{M}$;
%\emph{ii})~the electron temperature is inversely proportional to $\Rh$; and
%\emph{iii})~the models need to normalized so that $\langle F_{230}\rangle = 2.4\Jy$.
...
% cfg 2/9: removed this as unhelpful even to the knowledgeable reader.  it conflates the definition of jet and the definition of outflow power.
%: \citet{refId0}, \citet{2014A&A...570A...7M} identify the jet as the region above a threshold Bernoulli parameter Be, while \citet{10.1111/j.1365-2966.2012.22002.x} define it as the region with the ratio of energy flux to rest mass flux $\mu$ above a critical value, and \citetalias{M87PaperV} applies a $\beta\gamma$ cut to define the jet $P_\mathrm{out}$.  
\begin{equation}
...
We have tested a large set of nonthermal models, which have a power-law tail on the eDF.
...
First, the 230~GHz images are relatively insensitive to the presence of nonthermal electrons for models in which most of the nonthermal electrons are introduced in and near the outflow region.
...
% cfg 2/5: this seems really different in tone and emphasis from the rest of the paper.  I think it could be reduced in length.  
...
General relativistic (radiative) kinetic simulations are crucial for dynamically probing the electron temperature, effects of nonthermal distribution functions, and pressure anisotropy and their interplay with radiation in collisionless plasma in the accretion disk and jet.
...
The increase in $2.2\um$ flux density with nonthermal population seen in Section~\ref{sec:comparisons} is consistent with this notion.
