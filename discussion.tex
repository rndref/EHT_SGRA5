...
\end{equation}
where $poles$ means $\theta<1$ or $\theta>\pi-1$. The integral is evaluated at $r=100GM/c^{2}$ and we consider only those zones where there is outflow, i.e. the quantity in the parentheses is positive. $P_{out}$ includes power in the relativistic Poynting jet, if present, and in the slower, denser disk wind.
