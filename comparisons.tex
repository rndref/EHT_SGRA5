...
We consider two flavors of the model around $\abh = 0$ black holes: one in which the stellar wind magnetization is low ($\beta = 10^6$ and another and a second in which the magnetization is higher ($\beta = 10^2$).  The $230$GHz flux from each model is adjusted by adjusting $\rhigh$ until the correct mean flux is obtained.
