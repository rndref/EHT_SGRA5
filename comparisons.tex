...
Why might the $\mi{3}$ constraint be wrong?  One possibility is that there is flux from extended structure that is slowly varying, captured in the light curves, and resolved out by EHT.  The required extended flux is model-dependent, but if $\sim 50\%$ of the flux were in an extended component then nearly all the MAD models would pass $\mi{3}$, and many of the SANE models would be rejected as too quiet.
