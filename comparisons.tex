...
First we consider the consequences of departures from the $\Rh$ temperature assignment scheme in thermal models.  The $\Rh$ prescription has been the default parameterization, but there are many other possibilities.  If we still assume that the ion-electron temperature ratio depends only on current local conditions a well-motivated alternative is the critical beta model  \cite{2020MNRAS.493.1404A}, which sets $T_e = f (T_e + T_i) \exp(-\beta/\beta_c)$.  We consider only a single point in the critical beta parameter space, with $f = \beta_c = 1$. 

We have applied a subset of tests to the critical beta models (all except X-ray; NIR is calculated with imaging only and therefore does not include Compton scattering).  The full set of results is given in Appendix~\ref{app:tables}, Table~\ref{tab:betacritPF}. 

All Critical beta models with high inclinations fail the EHT constraints, which is consistent with our main conclusion about the thermal models.  We also find that all critical beta models fail all considered non-EHT constraints.
