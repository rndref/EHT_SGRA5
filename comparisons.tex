...
Variability is central to  the interpretation of \sgra: an 8\hr observation of \sgra is $1400 \tg$, a timescale over which most models vary significantly.  In contrast, an 8\hr observation of M87* is $\sim \tg$ and the source is hardly expected to vary at all.\footnote{This does not mean that \m87 is less variable than \sgra at comparable time scales.  In 2017 EHT observed \m87 over only $\sim 19\tg$, so it is not possible to characterize \m87 variability using 2017 EHT data alone.  To obtain variability information for \m87 similar to what we present here for \sgra would require multi-year observations.}
