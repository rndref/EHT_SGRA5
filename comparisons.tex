...
%------------------------------------------------------------------------------
\subsubsection{Thermal Plus $p_\mathrm{pl} = 4$ Power-law Models}
...
Next we consider a mixed thermal/nonthermal eDF, with the nonthermal component following the kappa DF.  We set $\kappa = 3.5$.  At high frequency $\nu L_\nu \sim \nu^s$ with $s = 2 - \kappa/2 = 1/4$, similar to what is seen in NIR flares \citep{2007ApJ...667..900H}.  The underlying GRMHD models are run with \bhac, and the radiative transfer is done with \bhoss.  

The fraction of nonthermal electrons is assumed to depend on $\sigma$, and $\beta$.  We can write the total emissivity as:
\begin{equation}
...
We set $\sigma_{\rm min}=0.01$ and vary the base efficiency, $\varepsilon$, between 0.05, 0.1 and 0.2. For each base efficiency we generate a model set spanning the same parameter space as the thermal models (see Table \ref{tab:radiativemodels} for details),
and normalize the accretion rate using the same procedure as the standard models.  
...
The addition of non-thermal electrons increases the NIR flux for all models.  In Fig.~\ref{fig:NIR_kappaepsilon} we show the distribution of the NIR fluxes for thermal and kappa eDF for SANE accretion models.
...
For SANE models, except $\Rh=1$, the addition of non-thermal particles leads to over-production of NIR. For MAD models, all models over-produce NIR for $\varepsilon \ge 0.05$.
...
We compute $mi{3}$ for a window of $5000\tg$.  Note that this is shorter than the standard models, so the results are less constraining.  Approximately 1/2 of models pass for $\varepsilon = 0.05, 0.1,$ and $0.2$.  This passing fraction is higher than for the thermal model set which includes 3 $5000\tg$ windows.   

% cfg 12 dec: awaiting results that will determine whether higher passing fraction in these models is a consequence of the shorter duration or the nonthermal electrons. 
...
\ckc{CK still working on it...}

\begin{figure*}
  \centering
  \includegraphics[width=\textwidth]{figures/bestbets.png}
  \caption{The visibility amplitudes and SEDs of the two selected
    possible models compared with data.
    In both panels, symbols in black are the observation constraints; blue is Model A ($\abh=0.5$ MAD, $\Rh=160$, $i=30$); and orange is Model B ($\abh=0.94$ SANE, $\Rh=40$, $i=30$).
    In the left panel, we show the visibility amplitudes as function of $u$-$v$ distance.
    For the observation data, we plot 10 second incoherent averaged data from the HOPS pipeline.
    For the models, we plot 8 different position angles for every snapshot.
    In the right panel, we show the SEDs for Model A and B and again compare them with the observation constraints.}
  \label{fig:bestbets}
\end{figure*}
