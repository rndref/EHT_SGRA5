...
\begin{figure*}
  ...
  \caption{%
    We demostrate how the second moment constraint work in this
    Figure.
    For each of the panels, the white ellipse represents the FWHM of
    the Gaussian fits.
    Hence, the white dashed shows the FWHM major axes, and an
    invisible line perpendicular to that is the FWHM minor axes.
    The two green circles mark the upper and lower limits of the
    allowed size.
    Given the relax criteria described in Section~\ref{sec:sz}, an
    image is deemed compliant as long as the white ellipse touch the
    region between the two green circle.
    The left panel is a feature model (see Section~\ref{sec:bestbets})
    that passes this constraint because the white ellipse falls
    between the two green circle.
    The right panel is zero spin SANE model with $i=10$ and $\Rh = 1$,
    that fails this constraint because even its minor axis is too big
    compare to the upper observed limit.
  }
...
The second moment constraint rejects only 1.5\% of models: nearly all models are about the right size.  The rejected models are $\abh \le 0$, face-on, SANE models with $\Rh = 1$. These models have extended emission with angular extent that is large compared to the critical impact parameter $b_c = \sqrt{27} \rg$.
...
\begin{figure*}
  ...
  \caption{
    \ckc{Rewrite and compare failing and pass models.}
...
\begin{figure*}
  ...
  \caption{\ckc{Add a two panel figure show a model fail and a model pass all the non-EHT constraints.}}
