...
Figure \ref{fig:cmp_VLBI_var} shows the distribution of $(\sigma_\text{var}^2, b)$ from the EHT observation, along with the distribution across all \kharma models. The GRMHD models are shown as an aggregate whole, but each model consists of only three measurements of $\sigma_\text{var}^2$, one on each window. This makes a direct comparison with the measured value difficult, as the 
...
So far we have considered a Maxwell-J\''uttner distribution function that describes a thermal population of electrons. Next we describe our results from 
...
The total flux for SANE models with R$_{\rm high}=1$ slightly increases, all them fail. While for $R_{\rm high}=10,40,80$ and $R_{\rm high}=160$ the flux decreases bellow $1.8 \rm {Jy}$.

%..............................................................................
\subsubsubsection{NIR constraints}

MAD:
$R_{\rm high}=1$ only $a_{\star}=0$ and $i=10^{\circ}$ pass in contrast with thermal models.

All models with electron  temperature with R$_{\rm high}=10,40$ fail, again in thermal model this not happening ...

$R_{\rm high}=10,40$ only $i=10^{\circ}, 30^{\circ}$ pass,
$R_{\rm high}=80$ for spin zero pass pass and for $a\neq 0$ only inclinations $30^{\circ} \leq i$.
$R_{\rm high}=160$ only $i>50^{\circ}$ fail.

SANE:

All $R_{\rm high}=10$ and $a_{\star}=-0.94, -0.5$ fail
All $R_{\rm high}=40$ and $a_{\star}=-0.94, -0.5,0.0$ fail, similar trend for $R_{\rm high}=80$.
$R_{\rm high}=160$ we have a bit higher emission but pass the constraint as in thermal case ...
