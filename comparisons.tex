...
The distribution of power-law index $b$ of the variance $\sigma_\text{var}^2 (|u|)$ at $4~{\rm G}\lambda$ for the GRMHD models is generally in \aeb{\sout{rough} good} agreement with the measured slope from the 2017 EHT campaign (excluding April 11). The amplitude $\afour^2$, however, varies depending on the model and the code.
...
We consider two versions of the model: one in which the stellar wind magnetization is low ($\beta = 10^6$ and another and a second in which the magnetization is higher ($\beta = 10^2$). The $230$~GHz flux from each model is fixed by adjusting $\rhigh$ until the correct mean flux is obtained. The resulting $\Rh = 13$ ($\beta = 10^6$) and $\Rh = 28$ ($\beta = 10^2$).
...
Interestingly both non-EHT and EHT constraints have the power to test the wind-fed model.  We cannot draw a broad conclusion about the viability of wind-fed models, however: these two models include only a single spin and a single model for the eDF.
