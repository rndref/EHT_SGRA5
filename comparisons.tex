...
\begin{figure}
  ...
  \caption{Null Location Constraint\ckc{Texts/labels in pizza plots too small to read.}\monika{we should move this pie charts to Appendix Table and day that this is a visual representation of the tables, they can be made larger in the appendix. In the main text we show only eht,non-eht, joint (also in full page width mode)}}
  \label{fig:cmp_ozel}
\end{figure}
...
Variability is central to interpretation of \sgra: the small black hole size means that observations considered here are taken over intervals when the source is expected to vary significantly.  This distinguishes \sgra from \m87, where the source is expected to vary only over timescales that are long compared to a single track.\footnote{This does not mean that \m87 is less variable than \sgra.  In 2017 EHT observed \m87 over only $\sim 15\tg$, so it is not possible to characterize \m87 variability using EHT data alone.  To obtain variability information for \m87 similar to what we present here for \sgra would require multi-year observations.}
...
\begin{figure}
  ...
  \caption{Distributions of $\mi{3}$ for KHARMA models (black), compared to distributions from historical observations (gray) and measurements of $\mi{3}$ on independent segments from the 2017 ALMA light curve. The latter are shown as vertical lines. The distributions for models with the lowest (blue) and highest (red) average $\mi{3}$ for SANEs and MADs are also shown. 
...
The distribution of slopes $b$ of the variance $\sigma_\text{var}^2 (|u|)$ at $4~{\rm G}\lambda$ for the GRMHD models is in rough agreement with the measured slope from the 2017 EHT campaign (excluding April 11). \dl{need brief description of $b$ measurement in section 3.3.2} The amplitude of $\sigma_\text{var}^2 (4~{\rm G}\lambda)$, however, varies depending on the model and the code. 

Figure \ref{fig:cmp_VLBI_var} shows the distribution of $(\sigma_\text{var}^2, b)$ from the EHT observation, along with the distribution across all \kharma models. The GRMHD models are shown as an aggregate whole, but each model consists of only three measurements of $\sigma_\text{var}^2$, one on each window. This makes a direct comparison with the measured value difficult, as the distribution for a given model is poorly constrained. 

\citet{Georgiev_2022} gives an estimate for the width of the distribution as $\log_{10}(\sigma_\text{var}^2) \pm 0.1$. We can get a rough estimate for how the GRMHD models fare compared to the measurement by taking the mean across windows and the estimate for the width, and convolving this with the measurement distribution. Under this, 83/200 models (46 SANE and 37 MAD) are within 1\% of the measurement, although we caution against interpreting this number as the number of passing or failing models, since the uncertainties in the model distribution are so large. 
...
\begin{figure*}
  \centering
  \includegraphics[width=1.\textwidth]{./figures/All_Constraints.png}
  \caption{Combined EHT and non-EHT constraints\monika{if this plot is shown like this we need a slightly larger font for $a_*$ and bottom legend.}}
  \label{fig:all_cuts}
\end{figure*}
...
\begin{figure}
  ...
  \caption{Distribution of \mi{3} from the full \koral model set, from the first half, from the second half, and from the comparable Illinois thermal models.\monika{this figure has to have code labels updated}}
  \label{fig:koral_MI}
\end{figure}  
...
So far we have assumed a Maxwell-J\''uttner distribution function that describes a thermal population of electrons. Next we consider a non-thermal population of electrons using either a mixed thermal-powerlaw eDF or a pure/mixed kappa eDF.

\subsubsection{Constant power-law models $p=4.0$}

In this section, we employ a hybrid thermal/power-law tail distribution using H-AMR/BHOSS, and assume a power-law index of $p=4$ with a non-thermal acceleration efficiency $\epsilon=n_{\rm power-law}/n_{\rm thermal}=0.1$. The power-law is stitched to the thermal core by choosing the minimum Lorentz factor limit of the power-law $\gamma_{\rm min}$ to be at the peak of the Maxwellian. The maximum limit of the power-law is taken to $10^5 \gamma_{\rm min}$. Further details about the setup is given in Chatterjee et al. 2021. Similarly to the thermal models, we normalize the density scale such that the mean 230 GHz flux is 2.4 Jy over 30-35kM. We note that the normalization value is smaller than that of the corresponding H-AMR/BHOSS thermal models, suggesting that the power-law emission contributes to the 230GHz total intensity.

The addition of the power-law tail primarily increases the flux in the NIR and thus, the NIR GRAVITY flux of 1.0 mJy could provide a constraint on the power-law index and the acceleration efficiency. In brief, $R_{\rm high}=1$ MAD models are ruled out by the NIR constraint.
...
In addition to the aligned models, we also consider misaligned geometrically thick disks around black holes with a spin of $a_*=15/16$ taken from \citet{Liska2018} and \citet{Chatterjee2020}. Further, while the models in the SgrA* GRMHD simulation library exhibit either a SANE or MAD accretion flow, the tilted disk models assume a strongly magnetized non-MAD disk, otherwise known as an InSANE disk. Overall, we incorporate three GRMHD models of differing misalignment angles - $0^{\circ}$, $30^{\circ}$ and $60^{\circ}$. The models show a strongly warped disk and jets due to the Lense Thirring torque, and thus the disk/jet morphology is non-axisymmetric. Since the inner and the outer disk possess different orientations, it is necessary to specify the coordinate axis of the 

In brief, highly misaligned models are more variable that their aligned counterparts.
