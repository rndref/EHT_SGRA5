...
\begin{figure*}
  ...
  \caption{Visibility amplitude plots for demonstrating the null
    ...
    marks the allowed median value ($V \leq (4\%/50\%) V_0$) of the
    normalized visibility amplitude for long baselines.
    We numerically find the local visibility minima along each
    position angle, which are marked by solid points.
    ...
    A model is rejected if \emph{any} of these lines land
...
None of our standard thermal models (Frankfurt or Illinois) survive the full gauntlet of constraints.   Variability rejects about 3/4 of models and prefers SANEs, which are quieter than MADs.  The remaining constraints prefer MAD models.  The independent Frankfurt and Illinois model are in full agreement.   
