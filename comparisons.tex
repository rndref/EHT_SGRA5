...
We start with a set of ``standard'' models with aligned (prograde or retrograde) accretion flows, thermal eDFs, and electron temperature assigned according to the $\Rh$ model, as in \citetalias{M87PaperV}.  This selection includes the Illinois (i.e.\ \kharma), Frankfurt (i.e.\ \bhac), and \hamr model sets listed in Table~\ref{tab:radiativemodels}. Table~\ref{tab:passfraction_thermal} summarizes the fraction of the thermal models which pass each constraint.
...
\hline
Code/Setup						& MAD/SANE	&	spin	&	inc		&	$\Rh$	&	Constraint failed	\\
\hline
...
The 86\GHz source is only slightly affected by the addition of non-thermal particles as compared to the thermal models. Only the SANE models with $R_{\rm high}\geq 40$ and $a^{\star}>0$ produce 86\GHz image sizes larger than the thermal SANE models. This effect can be seen in  first and third panel in the bottom row of Fig.~\ref{fig:SANE_edfs}. Notice the increased flux density in the jet sheath in the difference image (blue color). This trend increases with the efficiency and is reflected in the decreasing pass fraction: 56\% (for $\varepsilon=0.05,0.1$) and 55\% ($\varpesilon=0.2$) as compared to thermal models (59\%).  
...
%------------------------------------------------------------------------------
\begin{table*}
\caption{Pass fractions for \bhac and \hamr models for varios eDFs}
\centering
\begin{tabular}{l|cccc|ccc}
\hline
 constraint/model	& \multicolumn{4}{c}{\bhac}        & \multicolumn{3}{|c}{\hamr} \\
 \hline
...
lc varability&  	  0.27	& 0.30	&  0.47,0.47,0.46	& 0.29  & 0.07	& 0.28	&  0.41\\
4G$\lambda$ variability & 0.72	& 0.60	&  0.74,0.73,0.71	& 0.55  & 0.39	& 0.46	&  0.63\\
Null location     &0.83	& 0.80	&  0.81,0.81,0.78	& 0.59  & 0.80	& 0.88	&  --\\	
M-ring diameter&0.65	& 0.69	&  0.66,0.66,0.67	& 0.71  & 0.61	& 0.67	&  0.89\\
M-ring width    & 0.21	& 0.21	&  0.24,0.23,0.23	& 0.03  & 0.32	& 0.40	&  0.13\\
M-ring asym.   &0.95		& 0.97	&  0.95,0.95,0.94	& 0.73  & 0.99	& 0.97	&  0.98\\
86\,GHz size   &0.59		& 0.57	&  0.56,0.56,0.55	& 0.38  & 0.46	& 0.45	&  0.47\\
86\,GHz flux    &0.68		& 0.75	&  0.67,0.66,0.63	& 0.12  & 0.62	& 0.65	&  0.72\\
2.2$\mu$m flux&0.55	& 0.35	&  0.14,0.12,0.12	& 0.14  & 0.80	& 0.25	&  0.41\\
x-ray flux& 	0.70		& --		    	& --		& --  	    & 0.61	&  0.35	& --\\
\hline
\end{tabular}
\label{tab:passfraction}
\end{table*} 
...
\newline The main characteristic of the non-thermal models is the increase of 2.2 μm and X-ray flux densities. However, in a large fraction of models this leads to overproduction of 2.2 μm or X-ray flux and reduces the pass fractions.\\

If all constraints are considered all non-thermal models fail. However there are a few models which only fail the lightcurve variability, $\mi{3}$. These models are: a \hamr MAD $p=4$ model with $a^\star=0.5$ seen under an inclination of 90$^\circ$ and $R_{\rm high}=1$ and a \bhac MAD variable $\kappa$ model with spin $a^\star=0.5$ inclination 30$^\circ$ and $R_{\rm high}=160$. These two models agree with the cluster of thermal models found for both \bhac and \kharma models (see Sect.~\ref{sec:summarythermal}).
\cmf{there is one \bhac SANE $\varpesilon=0.05$ with spin $a^\star=0.94$, inclination 10$^\circ$, and $R_{\rm high}=10$ which only fails M3. However this one would fail most likely the polarisation constraint. How do include this here? }
