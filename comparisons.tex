...
\begin{figure}
  ...
  \caption{Distributions of $\mi{3}$ for \kharma and \bhac models (black), compared to distributions from historical observations (gray). The distributions for models with the lowest (blue) and highest (red) average $\mi{3}$ for SANEs and MADs are also shown. The heights of these distributions have been scaled down for clarity.
  }
...
\end{align}
%\cmf{Since $\varepsilon=0$ we can simplify the $w$ and adopt the text below,or?}
...
\cmf{text from Alejandro: The post-processing radiative transfer is done employing the GRRT code \bhoss using GRMHD simulations from the \bhac. We use the time window from $\rm 25,000M-30,000M$ and we created images every $\rm 10M$ normalizing the average flux of $\rm 2.4\,Jy$ at $\rm 230\,GHz$.

%..............................................................................
\subsubsubsection{Null Location}
We found that $80\% ~(201/250)$  pass the expected null location ($2.5\lambda - 3.5G\lambda$). The differences to the thermal models can be described as follows: higher inclinations are rejected for $i \geq 70^{\circ}$ (MAD)  and $i \geq 50^{\circ}$ (SANE). Although, the total number of rejected models is similar to thermal models.

%..............................................................................
\subsubsubsection{4 \texorpdfstring{$G\lambda$}{Gl} Visibility Amplitude Variability}

In case of the variability at 4 $G\lambda$ 151/200 pass the constraint, same percent as for the thermal models, for instance $\sim 40 \%$ of the models are rejected, mostly negative $a_{\star} = -0.94$ SANE and MAD, also high temperatures, $R_{\rm high} = 160$, $a_{\star} \leq 0$ MAD accretion models, and  $i \geq 70^{\circ}$  for SANE prograde models.}
...
\aco{
The addition of nonthermal particles increases the x-ray emission from the majority of MAD models overshooting the constraints, but not changing the general behaviour of weak magnetic field SANE models.
%..............................................................................
%\subsubsubsection{86\GHz Major Axis}
...
The nonthermal eDF models considered here produce little change compared to the thermal models for most constraints.  The 86\GHz size and flux, which are the most important non-EHT constraints, are not detectably affected by the addition of nonthermal electrons.
The 230\GHz m-ring fits are hardly affected at all by the addition of nonthermal electrons.  And $\mi{3}$ is also not detectably affected by the addition of nonthermal electrons.  The NIR and x-ray flux density, however, is consistently increased by the nonthermal eDF prescriptions considered here.  In a large fraction of models this leads to overproduction of NIR or x-ray flux. Since the nonthermal models analysed here are not considering the heating efficiency due to the magnetic reconnection, $\varepsilon \neq 0$, those effects can be explained by asymptotic behaviour of the nonthermal emissivity, $ j_{\nu}\propto\exp\left(-\Theta_e^{-2/3}\nu^{1/3}\right)+\nu^{-(\kappa-2)/2} \left[(\kappa -3)\Theta_{\rm e} + \varepsilon(\kappa -3) \sigma\right]^{\kappa-2}$, the second term dominates at NIR and x-rays, while at mm and radio frequencies the thermal component, the first term dominates. However, we expect that including high efficiency heating of nonthermal particles the 86\GHz, the flux will increase two times and larger image size \citep{2021NatAs.tmp..218C,2021arXiv211102518F}. Furthermore, NIR and x-ray average flux increase as efficiency increase, on the other hand, the SED is not sensitive to the black hole spin but image size increase for prograde models, we have consistency overall our models (\bhac / \hamr).
...
Both non-EHT and EHT constraints have the power to test the wind-fed model.  It is {\em not} possible to draw broad conclusions about the viability of the wind-fed models, however, since the two models tested here contain only a single spin ($a_\star=0$) and all use the $\Rh$ thermal eDF model.

%% \begin{figure*}
%%   \centering
%%   \includegraphics[width=\textwidth]{figures/bestbets.pdf}
%%   % cfg 15-dec: edited for clarity, left all content in.
%%   \caption{Visibility amplitudes (left) and SEDs (right) of the three featured models, compared with the data.  Black symbols show observations.  Blue, orange, and green are Models~A, B, C, respectively (see Table~\ref{tab:bestbets} for details).  Observed VAs are 10\sec incoherently averaged data from the HOPS pipeline on \aprilvii.  Model VAs are shown as a solid line for a section at position angle $0\deg$ for a single snapshot, and the band shows the 1st through 99th percentile over all position angles and all times.  Model SEDs show a solid line for the mean SED and a band for the range across snapshots.
%%   }
%%   \label{fig:bestbets}
%% \end{figure*}
...
%%   % cfg 15-dec: edited down
%%   \caption{Example images from featured models.  The columns show models ~A, B, and C from Table~\ref{tab:bestbets}, the rows show the 86\GHz image, 230\GHz image, and 230\GHz image smoothed to EHT resolution from top to bottom.
%%     \ckc{Will update snapshot once receive the consensus image from PaperIII.}}
