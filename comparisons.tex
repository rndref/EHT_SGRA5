...
Most NIR photons are produced by the synchrotron process from electrons on the high energy end of the eDF.  For a one-zone model with $B = 30$G, the  critical frequency $\nu_{crit} \simeq \gamma^2 e B/(m_e c) \simeq 80$GHz and emission at $2.2\mu$m is therefore produced by electrons with  Lorentz factor $\gamma \simeq 10^3$; the mean $\gamma = 30$ for a thermal eDF with $\Theta_e = 10$.  NIR flux density will therefore be sensitive to $\Theta_e$ and therefore to $\Rh$, and to the magnetic field strength.
