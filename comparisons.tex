...
We show visually how some of the constraints work in
...
\begin{figure*}
  ...
  \caption{%
    Visual representation on how the \vam\ constraint work.
    ...
    left matches reasonablely well with the data; while the model on
    the right is complete off.
    The \vam\ constraint is exactly design to rule out models like the
    one in the right.
    ...
    For the examples in this figure, the left model passes the
    \vam\ constraint because one of the first null (green dot) falls
    within the allowed range; and none of the long-baseline median
    (all dotted lines) go above 4\%/50\%.
    The right model fails the \vam\ constraint obviously because both
    of the null location and long-baseline VAs are off.}
...
\begin{figure*}
  ...
  \caption{%
    \ckc{We may skip this figure given Fig.~\ref{fig:sample_seds} is
      similar.}
...
observation by comparing its null location and long-baseline visiblity
amplitude.
As Figure~\ref{fig:passfail_va} demonstrate, this constraint is
informative.
It tends to matches the ring size by using the null location, and the
ring width by using the long-baseline amplitude.
...
We will discuss option (1) later.  To investigate options (2) and (3), we can ask what happens when we lift one constraint.  About 5\% of both the Frankfurt and Illinois model sets fail only 1 constraint.  For Frankfurt, 8 models fail only $\mi{3}$, and 2 fail only m-ring diameter.  For Illinois, 3 models fail only $\mi{3}$ and 5 fail only the 86GHz size.  Suspicion therefore falls on $\mi{3}$, which rejects $\approx 70\%$ of Frankfurt and Illinois models.  Why might the $\mi{3}$ constraint be misleading?
...
\begin{figure*}
  ...
  \caption{%
    The visibility amplitudes and SEDs of the two feature models
    compared with data.
    In both panels, symbols in black are from observations; blue is
    Model~A ($\abh=0.94$ SANE, $\Rh=40$, $i=30$); orange is Model~B
    ($\abh=0.5$ MAD, $\Rh=40$, $i=30$); and green is Model~C
    ($\abh=0.5$ MAD variable $\kappa$, $\Rh=160$, $i=30$); see
    ...
    For the models, the solid curves are the VA for only the position
...
\begin{figure*}
  ...
  \caption{We show here the 86\GHz and 230\GHz ray traced images of
    the feature models.
    The different columns are Model~A, B, and C, respectively (see
    figure title or Table~\ref{tab:bestbets} for model description).
    The top row shows 86\GHz image, the middle row shows 230\GHz, and
    the bottom row shows the 230\GHz filtered by a Butterworth filter
    at 10$\,\mathrm{G}\lambda$, which approximate the resolution of
    the 2017 EHT array.
    On one hand, although Model~A looks different than Model~B and C,
    they pass all the constraints we described in this paper.
    On the other hand, Model~B and C, although one is thermal and the
    other is nonthermal, look remarkable similar at 230\GHz.
    The former point suggests that there is still ambiguity in the
    image morphology, consistent with \citetalias{PaperIII,PaperIV}'s
    conclusions.
    The later point suggests that even for the same image morphology,
    there are still degeneracy in the eDF, suggesting the importance
    of new multiwavelength observations.}
...
\ckc{CK to work on sample plots, may add extra feature models upon
  requests.}
