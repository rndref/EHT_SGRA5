...
\begin{figure*}
  ...
  \caption{%
    230\GHz full resolution snapshots from a fiducial model from the
    best-bet region.
    This model passes 10/11 constraints.
    The different panels are snapshots taken from the time windows
    that the sythetic observation corresponding to April 7
    observations has good \uv coverage.}
...
\begin{figure*}
  ...
  \caption{%
    Reconstructions of the fiducial model from the best-bet region,
    compared to the observation.
    This is the same model shown in
    Figure~\ref{fig:bestbet_imgs_snapshot}.
    The leftmost panel is an average of the snapshots used in the
    sythetic data generation, weighted by the number of baselines.
