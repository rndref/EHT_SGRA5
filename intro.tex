...
\ckc{It's good to be inclusive in citations but we need to make the intro readable.  I suggest only keep the i) first few, ii) ``classic'', and iii) review papers in the intro, and say things ``see ... and reference therein''.  Citation of newer work can then be moved into the model description sections.  For example, when we introduce the variable kappa, we can cite the more recent work.  This way we cite EHT members' recent work but don't make the intro unreadable.}
